...
options(timeout = max(6000, getOption('timeout')))   # for download.file
...
if (length(args) < 9) {
    stop("Please provide the arguments needed for the successful execution of this script.
        Arg1: path where data is.
        Arg2: select the option of the satellite used (OCO-2, OCO-3, TROPOMI, NA)
        Arg3: select the version of the satellite data (V11r, V10p4r, NA)
        Arg4: select the species of the satellite data (CO2, CO, NO2, CH4))
        Arg5: multiple times or single time (TRUE/FALSE)
        Arg6: create overpass files (TRUE/FALSE)
        Arg7: error flag (TRUE/FALSE)
        Arg8: run failed jobs (TRUE/FALSE)
        Arg9: city_choice")
}
input_path <- args[1]

sensor_input= as.integer(args[2])
version_input= as.integer(args[3])
species_input= as.integer(args[4])
multi_time = as.logical(args[5])
create_overpass_files_only = as.logical(args[6])
error_estimates = as.logical(args[7])
run_failed_jobs = as.logical(args[8])
city_choice = as.character(args[9])
...
# original flags
if (error_estimates==FALSE){
    cat('Running X-STILT without error estimates...\n')
    ...
    ## flags for generating error analysis
} else {
    cat('Running error analysis...\n')
    run_trajec    = T    # if to generate trajec; T: may overwrite existing trajec
                            ## run first with T to generate error analysis trajectories
    run_slant     = F    # recalc recp loc based on SZA, VZA, AZA by obs geometry
    run_foot      = T   # if to generate footprint, set T if run_trajec is T
    run_hor_err   = T    # T: set error parameters
    run_ver_err   = T    # T: set error parameters
    run_emiss_err = F    # T: get XCO2 error due to prior emiss err
    run_wind_err  = T    # T: calc wind error based on RAOB, set T if run_hor_err = T
    run_sim       = F    # T: calc XFF or error with existing foot (only for CO2)
    nhrs          = -24  # number of hours backward (-) or forward (+)
}
...
obs_sensor  = c('OCO-2', 'OCO-3', 'TROPOMI', NA)[sensor_input]

obs_ver     = c('V11r', 'V10p4r', NA)[version_input]       # retrieval algo ver if there is
obs_species = c('CO2', 'CO', 'NO2', 'CH4')[species_input]  # only 1 gas per run
...
out_path="/uufs/chpc.utah.edu/common/home/lin-group26/CarlXav"

# create overpass files first to get all the overpasses with qf. The next step is to run them 
if (create_overpass_files_only) {
    cat("in create_overpasses_files_only")
    city_list <- file.path(xstilt_wd, "Cities_input_files/CONUS_cities_list.txt")  # Replace "data.txt" with your file name
    CONUS <- read.table(city_list, header = TRUE, sep = "\t")  # Assuming tab-separated values with headers

    # save the receptors, site and time string to the overpass folder
    out.txt.path = file.path(dirname(oco_path), 'overpass_city') 
    recp.out.fn = file.path(out.txt.path, paste0('nrecp.txt'))

    # city=c("SanFrancisco_SanJose")  
    city=c("Indianapolis", "Columbus","Cleveland")  
    ...
    for (ss in city) {
        ...
        # out_path="/uufs/chpc.utah.edu/common/home/lin-group26/CarlXav"
        out_path2="/scratch/general/vast/u6065567"
        lon_lat = get.lon.lat(site, dlon, dlat, site.loc = NULL)
        store_path = file.path(input_path, 'XSTILT_output', site) # lin-group20
        # store_path = file.path(out_path2, 'XSTILT_output', site)
        print(paste0("store_path is:: ",store_path))
        ...
        cat('Creating overpass files only...\n')
        timestr = get_timestr(site, lon_lat, obs_sensor, obs_ver, obs_path, 
                            ...
                            date_range = c('20150101', '20241231'), multi_time=multi_time,
                            create_overpass_files_only=create_overpass_files_only)
        cat('Done with choosing cities & overpasses...\n')
        # get number of recptors
        

        for (ii in 1:length(timestr)) {
            
            n_recp = get.number.receptors(timestr[ii], 
                                  obs_fn, obs_sensor, obs_path, obs_species, 
                                  lon_lat, numpar, 
                                  nf_dlat = nf_dlat, 
                                  nf_dlon = nf_dlon, 
                                  num_bg_lat = num_bg_lat, 
                                  num_bg_lon = num_bg_lon, 
                                  num_nf_lat = num_nf_lat, 
                                  num_nf_lon = num_nf_lon, 
                                  agl, run_trajec, run_slant)
                               
                                # Save n_recp, timestr, and site in a dataframe
            nrecp_df <- data.frame(
                site = site,
                timestr = timestr[ii],
                n_recp = nrow(n_recp)
            ...
            # Append the dataframe to a file
            
            
            write.table(nrecp_df, file = recp.out.fn, sep = ",", row.names = FALSE, quote = F,
                            append = file.exists(recp.out.fn), col.names = !file.exists(recp.out.fn))
            cat(paste('Done with receptor setup...total', nrow(n_recp), " for timestring ", timestr[ii], 
               ' receptor(s)..\n\n'))
        }
    ...
    out_path2="/scratch/general/vast/u6065567"
    # out_path2="/uufs/chpc.utah.edu/common/home/lin-group26/CarlXav"
    if (run_failed_jobs) {
    #    overpass_to_model <- read.table(file.path(OCO.DIR, "/OCO-2/overpass_city/incomplete_log_v1.txt"),
    #             header = TRUE, sep = "\t")  
    ...
    # site=c("Chicago")
    site=city_choice
    print(paste0("Processing city: ", site))
    # for (ss in CONUS$City) {
    for (ss in site) {
    ...
    # - OR mannually insert coordinates, e.g., site.loc = data.frame(lon = , lat = )
            ...
            # store_path = file.path(out_path, 'XSTILT_output', site)
            # store_path = file.path(out_path2, 'XSTILT_output', site)
            print(paste0("store_path: ",store_path))
            if (!is.na(obs_ver)) store_path = file.path(store_path, obs_ver)
            print(oco_path)
            ...
            if (site=="Miami"){
                raob_path  = file.path('/uufs/chpc.utah.edu/common/home/lin-group14/KaiW/XSTILT_OCO/X-STILT/radiosonde/East_FL')
            } else {
                raob_path  = file.path('/uufs/chpc.utah.edu/common/home/lin-group14/KaiW/XSTILT_OCO/X-STILT/radiosonde',site)
            }
            # raob_path = file.path('/uufs/chpc.utah.edu/common/home/lin-group14/KaiW/XSTILT_OCO/X-STILT/radiosonde',site)
            raob_fn    = list.files(raob_path, 'tmp')
            odiac_ver  = c('2019', '2022', '2023')[3]         # ODIAC version
            odiac_path = file.path('/uufs/chpc.utah.edu/common/home/lin-group20/CarlXav/ODIAC_2021', odiac_ver)  
            # --------------------------------------------------------------------------- #
            ...
            met_path = file.path("/scratch/general/vast/u6036966/hrrr")
            # met_path = file.path("/uufs/chpc.utah.edu/common/home/lin-group21/hrrr/hrrr")
            # met_path = file.path("/uufs/chpc.utah.edu/common/home/u0791084/lin-group21/hrrr/")
            # met_path1 = file.path("/scratch/general/vast/u6036966/hrrr")
            # met_path2 = file.path("/uufs/chpc.utah.edu/common/home/lin-group21/hrrr/hrrr/")
            # met_path = c(met_path1, met_path2)
            # met_path = file.path("/uufs/chpc.utah.edu/common/home/lin-group12/hrrr/hrrr/")
            # met_path = file.path(homedir, met)
            if (!dir.exists(met_path)) {
            dir.create(met_path, recursive = TRUE)
            } else {
            print('Met path already exists')
            print(met_path)
            }
            ...
            time_integrate = FALSE   # F: hourly foot; T: time-integrated foot
            ...
            # ct_ver      = ifelse(substr(timestr, 1, 4) >= '2016', 'v2017', 'v2016-1')
                ct_ver='2024-5'
                ct_path     = file.path('/uufs/chpc.utah.edu/common/home/lin-group20/CarlXav', 'CT-NRT', ct_ver)
                ctflux_path = file.path(ct_path, 'fluxes/')
                ctmole_path = file.path(ct_path, 'molefractions/')
                print(ctflux_path)
                print(ctmole_path)
            ...
            timeout = 16 * 60 * 60              # time allowed before terminations in sec
            job_time = '16:00:00'               # total job time
            # slurm_account = 'kingspeak-guest'         # slurm account
            # slurm_partition = 'owner-guest'
            ...
            namelist = list(ak_wgt = ak_wgt, ct_ver = ct_ver, ctflux_path = ctflux_path, 
                            ...
                            pwf_wgt = pwf_wgt, raob_fn = raob_fn, raob_path = raob_path, recp_fn = recp_fn, 
