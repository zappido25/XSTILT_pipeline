...
###  notes 11/08/2022
# definition of the city enhanced region too liberal in some cases
# need to interpolate for missing latitude bands to make this more reasonable
...
overpass_values<-function(city, timestr){
  library(stringr);library(dplyr);library(rioja);library(stats)
  
  #### get latitude range of interest for the city
  lat_range<-read.table("/uufs/chpc.utah.edu/common/home/lin-group14/KaiW/XSTILT_OCO/city_selection/city_clustering/urban_core_info.txt", header=T, stringsAsFactors = F)
  ...
  group<-read.table("/uufs/chpc.utah.edu/common/home/lin-group14/KaiW/XSTILT_OCO/surviving_simulations_10192022.txt", header=T, stringsAsFactors = F)
  group<-group$group[group$city == city & group$timestr == timestr]
  dir<-paste0("/uufs/chpc.utah.edu/common/home/", group, "/KaiW/XSTILT_OCO/OCO_obs/XSTILT_output/", city, "/out_", timestr,"_era5_OCO-2/by-id")
  
  #### get observations
  obs_file<-paste0("/uufs/chpc.utah.edu/common/home/lin-group14/KaiW/XSTILT_OCO/OCO_obs/overpass_obs/",city,"_", timestr,".txt")
  ...
  #### get nhrs
  nhrs<-read.table("/uufs/chpc.utah.edu/common/home/lin-group14/KaiW/XSTILT_OCO/nhrs_info_plusVisual.txt", header=T, stringsAsFactors = F, skip=3)
  nhrs<-nhrs$nhrs[nhrs$city == city & substr(nhrs$overpass,5,14) == timestr]
  ...
  foot_files<-dir(dir, recursive =T, pattern = "shit", full.names=T)
  ...
  # have to look these up too
  table<-read.table("/uufs/chpc.utah.edu/common/home/lin-group14/KaiW/XSTILT_OCO/err_status/completed.dat", header=T, stringsAsFactors = F)
  base1<-"/uufs/chpc.utah.edu/common/home/"
  base2<-"/KaiW/XSTILT_OCO/OCO_obs/error_output/"
  err_group<-try(table$V3[table$V1 == city & table$V2 == timestr], silent=T)
  #if(grepl("Error", err_group) ==T){
  #  err_group<-NA
  #}
  if(length(err_group) ==0){err_group<-NA}
  if(is.na(err_group) ==F){
    err_dir<-paste0(base1, err_group, base2,city,"/outerr_",timestr,"_era5_OCO-2")
    err_files<-dir(err_dir, recursive =T, pattern = "trans", full.names=T)
  ...
  # 19 simulations keep failing to generate values
  if(is.na(err_group)){
    err_df<-data.frame(lat_bin = 1:62, mean_err_pps = rep(0, 62))
  }
  ...
  if(grepl("Error", test) == TRUE){enh <-NA}
  
  if(grepl("Error", test) == FALSE){
  ...
  if(is.na(enh) == F & plot_flag == T){
    plot_flag<-F
    png(paste0("/uufs/chpc.utah.edu/common/home/lin-group14/KaiW/XSTILT_OCO/plots/",city,"_",timestr,".png"))
  ...
  write.table(enh, file=paste0("/uufs/chpc.utah.edu/common/home/lin-group14/KaiW/XSTILT_OCO/enhancements_lights/",city,"_",timestr,".txt"), quote=F, row.names = F)
  write.table(final_sects, file=paste0("/uufs/chpc.utah.edu/common/home/lin-group14/KaiW/XSTILT_OCO/sectoral_lights/",city,"_",timestr,".txt"), quote=F, row.names = F)
...
for(i in 1:21){
  print(i)
  overpass_values(df$city[i], df$timestr[i])
}
