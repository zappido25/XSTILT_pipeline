...
edgar_func_nhrs_v7_yearly<-function(foot, foot_byid, nhrs, city,shp_file){
  ...
  # going to do interpolation myself to avoid weekend/weekday issues in the downscaling code
        ...
        sectors<-c("ENE", "REF_TRF", "IND", "TNR_Aviation_LTO","TNR_Aviation_CDS",
        "TNR_Aviation_CRS", "TRO_noRES", "TNR_Other", "TNR_Ship", "RCO", 
        "PRO", "NMM","CHE","IRO", "NFE","NEU","PRU_SOL")
        sectors_downscaled_name<-c("ENE", "REF", "IND", "TNR_Aviation_LTO","TNR_Aviation_CDS",
        "TNR_Aviation_CRS", "TRO_noRES", "TNR_Other", "TNR_Ship", "RCO", 
        "PRO", "NMM","CHE","IRO", "NFE","NEU","PRU_SOL")
        sectors_v7<-tolower(sectors)
        ...
        # print(unique(interp_downscale$V2))
  ...
  # looks like emissions are in kg m-2 s-1 average over the month 
        ...
        edgar<-dir("/uufs/chpc.utah.edu/common/home/lin-group20/CarlXav/Data/EDGAR_v7/EDGARv7_Glb_0.1x0.1_anthro_CO2_excl_short-cycle_org_C__yearly",
        pattern = paste0(foot_year),
        full.names=T)
        sector_conv<-NULL

            
        for(e in edgar){
          print(paste0("Loading emissions for: ", e))
          # get sector and name the sector
          name<-substr(basename(e), 1, nchar(basename(e))-10)
          
          for (ii in 1:length(sectors_v7)) {
  # Load the sector variable as a raster brick from the NetCDF file
            emissions <- brick(e, varname = sectors_v7[ii])
            sect <- unlist(strsplit(sectors[ii], "_"))[1]
            print(paste0("Loaded emissions for sector: ", sectors_v7[ii], " ",sectors[ii], " ", sect))
          
            e_down<-interp_downscale[interp_downscale$V2 == sect,]
            # e_down<-interp_downscale[interp_downscale$V2 == sectors[ii], ]
            # Now you can use 'emissions' for further processing
          ...
          # need to convert to umol (44.01 g/mol) - need umol/kg
            ...
            sector_conv<-rbind(sector_conv, c(sectors[ii], convolution))
        }
     }
...
edgar_func_nhrs_v7_yearly<-Vectorize(edgar_func_nhrs_v7_yearly)
