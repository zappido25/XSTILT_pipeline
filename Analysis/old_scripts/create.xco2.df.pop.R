...
create_df <- function(obs,dir) {
  ...
  for (folder in folders) {
    ...
    smurf_file<- file.path(folder, paste0("bio_nhrs.txt"))
    bio=read.table(smurf_file, header=T, stringsAsFactors = F)
    
    pps_file<- file.path(folder, paste0("pps.txt"))
...
binning <- function(xco2_data, bin_size,dir, timestr) {
    ...
    # bin data df contains the statistics for each bin
    bin_data <- xco2_data %>%
      group_by(lat_bin) %>%
      summarise(
   ...
   # only use urban data for background receptors calculation
    ...
    bin_data_file <- file.path(dir, paste0("bin_data_pop_",timestr,".csv"))
